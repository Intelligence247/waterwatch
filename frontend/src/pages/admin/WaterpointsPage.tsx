...
/** Human-readable label for each phase of location acquisition. */
const phaseLabel: Record<LocationPhase, string> = {
...
export default function WaterpointsPage() {
  ...
  /**
   * Main "Get my location" handler.
   * Uses the new captureBestPosition with progress reporting.
   * Network fallback is now handled inside captureBestPosition itself.
   */
  const handleGetMyLocation = async () => {
    ...
    try {
      ...
      // captureBestPosition already tried network fallback internally.
      // Only reaches here on permission denied or total failure.
    ...
    if (editingId) {
      try {
        ...
        if (result.duplicateReviewWarning) {
          ...
          toast('warning', `Nearby waterpoint found (${distance}m): "${result.duplicateReviewWarning.conflict.name}". Flagged for review.`);
        }
      } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <a href="/admin/dedupe" className="inline-flex mt-2 text-xs font-semibold text-amber-700 hover:text-amber-800">
                    ...
                    <tr key={wp.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openEdit(wp)} className="p-2 rounded-lg text-slate-400 hover:text-teal-700 hover:bg-teal-50 transition-colors" title="Edit">
                            <Pencil className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeleteTarget(wp)} className="p-2 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Delete">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Latitude" required>
                  <input
                    ...
                    onChange={(e) => { setForm({ ...form, latitude: e.target.value }); setCapturedAccuracyMeters(null); }}
                    className="field-input" placeholder="8.4833"
                  />
                </Field>
                <Field label="Longitude" required>
                  <input
                    type="number" step="any" value={form.longitude}
                    onChange={(e) => { setForm({ ...form, longitude: e.target.value }); setCapturedAccuracyMeters(null); }}
                    className="field-input" placeholder="4.5653"
                  />
                </Field>
              </div>
              ...
              <div className="flex flex-wrap justify-end gap-2">
                ...
                >
                  {locating ? (
                    <>
                      ...
                      {locationPhase === 'gps' ? (
                        <span className="flex items-center gap-1">
                          <Satellite className="w-3.5 h-3.5" /> GPS warm-up…
                        </span>
                      ) : locationPhase === 'wifi' ? (
                        <span className="flex items-center gap-1">
                          <Wifi className="w-3.5 h-3.5" /> Wi-Fi fix…
                        </span>
                      ) : (
                        'Locating…'
                      )}
                    ...
                    <>
                      <LocateFixed className="w-4 h-4" /> Get my location
                    </>
                  )}
                </button>
                <button
              ...
              {/* Live phase description */}
              {locating && locationPhase && (
                <p className="text-xs text-teal-600 animate-pulse -mt-1">
                  {phaseLabel[locationPhase]}
                  {locationPhase === 'gps' && ' GPS can take up to 15 s on first use.'}
                </p>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Community" required>
                  <input
                    ...
                    className="field-input" placeholder="e.g. Adewole"
                  />
                </Field>
              ...
              <Field label="Photos (optional, max 5)">
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {form.photoUrls.map((url) => (
                        <div key={url} className="rounded-xl overflow-hidden border border-slate-200/60">
                          <img src={url} alt="Water point" className="w-full h-28 object-cover" />
                        </div>
                      ))}
                    </div>
                  ) : (
                  ...
                  />
                  {uploadingPhoto && <p className="text-xs text-slate-500">Uploading images…</p>}
                </div>
              </Field>
            </div>
            ...
            <div className="p-4">
              ...
              <div className="rounded-xl overflow-hidden border border-slate-200" style={{ height: 280 }}>
                ...
                >
                  ...
                  {mapPickerMarker && <Marker position={[mapPickerMarker.lat, mapPickerMarker.lng]} />}
                </MapContainer>
              </div>
              <div className="flex justify-end gap-2 mt-3">
                <button type="button" onClick={() => setMapPickerOpen(false)} className="px-4 py-2 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50">
                  Cancel
                </button>
                <button type="button" onClick={confirmMapPicker} className="px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold">
                  Use this point
                </button>
              </div>
            </div>
